...
export class User extends Model {
  ...
  static get relationMappings() {
    return {
      ...
      photo_id: {
        relation: Model.HasOneRelation,
        modelClass: UserPhotoId,
        join: {
          from: "user.id",
          to: "user_photo_id.user_id"
        }
      }
    };
  }
  ...
  async addUserPhotoId(params) {
    let res;
    const flag = await returnId(
      knex("user_photo_id").where("user_id", "=", params.id)
    );
    if (flag.length == 0) {
      res = await returnId(
        knex("user_photo_id").insert({
          user_id: params.id,
          image: params.image
        })
      );
  ...
  async getPhotoId(id) {
    let res = camelizeKeys(await UserPhotoId.query().findById(id));
    return res;
  }
...
class UserPhotoId extends Model {
  static get tableName() {
